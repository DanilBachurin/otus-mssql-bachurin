...
При обработке очереди создавайте отчет по количеству заказов (Orders) по клиенту 
...
CREATE TABLE CountOrdersCustomers(
...
ALTER AUTHORIZATION    
   ON DATABASE::WideWorldImporters to [DESKTOP-DH71HA4\Danil] ;
...
BEGIN
	...
	IF (@countOrderID > 0)
	BEGIN
		INSERT INTO CountOrdersCustomers (countOrder, Customer_id, Invoice_id, OrderDate, RecordDate)
...
FROM Sales.Invoices
WHERE InvoiceID IN (64334,64335,64336,64337) ;
...
drop table CountOrdersCustomers
