...
from OPENROWSET(bulk 'C:\Курсы\StockItems.xml', SINGLE_CLOB) as data;
...
set @xmlStockItems = (
		select *
		from OPENROWSET(bulk 'C:\Курсы\StockItems.xml', SINGLE_BLOB) as d
		);
...
drop table if exists #tStockItems;

/*
2. Выгрузить данные из таблицы StockItems в такой же xml-файл, как StockItems.xml
*/
declare @qStockItems nvarchar(max) = 'SELECT [@Name] = [StockItemName], [SupplierID],[Package] = (cast((SELECT [UnitPackageID], [OuterPackageID], [QuantityPerOuter], [TypicalWeightPerUnit] FROM [WideWorldImporters].[Warehouse].[StockItems] sip WHERE si.[StockItemID] = sip.[StockItemID] FOR XML PATH ('''''''')) as XML)),[LeadTimeDays],[IsChillerStock], [TaxRate], [UnitPrice] FROM [WideWorldImporters].[Warehouse].[StockItems] si FOR XML PATH (''''Item''''), ROOT (''''StockItems'''')';
declare @DynSQL nvarchar(max) = 'EXEC xp_cmdshell ''DynSQL "' + @qStockItems + '" queryout "C:\Курсы\StockItems1.xml" -c -t -T''';
